...
### クライアント証明書およびクライアント鍵
Secure Gateway クライアントがそれ自身をリソースに識別させる方法を指定する必要がある場合、クライアントが使用するための証明書および鍵をアップロードできます。クライアントとリソースは同じマシン上で実行されているため、これらを空のままにすることができます。そうすると、Secure Gateway サーバーがペアを自動的に生成してくれます。リソースが別のホスト上にある場合は、[アップロードするための証明書/鍵ペアを生成](./securegateway_keygen.html)する必要があります。

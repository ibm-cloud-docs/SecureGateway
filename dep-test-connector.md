...
1. Determine which IBM Cloud region you want to create your Connector in. Previously, you examined into which regions your Secure Gateway was deploymed. You don't have to pick the same region, but you probably want to.
...
{: step}
Although you were able to validate and test each part of the setup during each previous steps, let's review them to ensure that the parity between Secure Gateway and Satellite Connector matches. At the very end, you want to know that all data that we beign transfereed via Secure Gateway, is now properly being transfered via Satellite Connector.

1. Validate the Satellite Connector set up
- IBM Cloud Console: 
- IBM Cloud CLI: 

1. Validate a connected Agent
- IBM Cloud Console: 
- IBM Cloud CLI: 
...
When you are all done setting up Satellite Connectors to replicated your Secure gateway deployment, the only step left is to stop your Secure Gateway clients and disable delete your Secure Gateway gateways. When that is done your migration is complete.

...
- Determine if you want to run Secure Gateway and Connector concurrently. It is possible to set up Satellite Connector along side your Secure Gateway infrastructure and run them at the same time. To do this, you need to understand the data that is being sent over both the Secure Gateway client and the Satellite Connector agent. Either ensure that parallel data is being sent and set up some kind of duplicate data detection  to examine and handle the data properly. Or, you could configure some of the data to go over the Secure Gateway connection, and other data to go over the Satellite Connector connection.
